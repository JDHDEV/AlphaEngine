...
TiXmlElement* PhysicsComponent::VGenerateXml(void)
{
    ...
    TiXmlText* pShapeText = AC_NEW TiXmlText(m_shape.c_str());
    pShape->LinkEndChild(pShapeText);
	pBaseElement->LinkEndChild(pShape);

    // density
    TiXmlElement* pDensity = AC_NEW TiXmlElement("Density");
    TiXmlText* pDensityText = AC_NEW TiXmlText(m_density.c_str());
    pDensity->LinkEndChild(pDensityText);
    pBaseElement->LinkEndChild(pDensity);

    // material
    TiXmlElement* pMaterial = AC_NEW TiXmlElement("Material");
    TiXmlText* pMaterialText = AC_NEW TiXmlText(m_material.c_str());
    pMaterial->LinkEndChild(pMaterialText);
    pBaseElement->LinkEndChild(pMaterial);

    // rigid body transform
    TiXmlElement* pInitialTransform = AC_NEW TiXmlElement("RigidBodyTransform");
    ...
    TiXmlElement* pOrientation = AC_NEW TiXmlElement("Orientation");
...
void PhysicsComponent::VUpdate(int deltaMs)
{
    ...
    {
        AC_ERROR("No transform component!");
        return;
    }
...
/*
        AC_LOG("Actor", "Acceleration: " + ToStr(accelerationToApplyThisFrame) + "; velocityScalar: " + ToStr(velocityScalar) + \
...
*/
    ...
    if (m_angularAcceleration != 0)
    {
        ...
        // [rez] Comment this back in if you want to debug the physics thrust & rotation stuff.  It spams quite 
        // a bit of info the output window so I'm commenting it out for now.
        //AC_LOG("Actor", "Angular Acceleration: " + ToStr(angularAccelerationToApplyThisFrame) );
    }
}
...
void PhysicsComponent::RotateY(float angleRadians)
{
    ...
    if (pTransformComponent)
    {
        ...
        AC_ERROR("Attempting to call RotateY() on actor with no transform component");
}
...
void PhysicsComponent::SetPosition(float x, float y, float z)
{
    ...
    if (pTransformComponent)
    {
        ...
        AC_ERROR("Attempting to call RotateY() on actor with no trnasform component");
}
