...
const char* BrickRenderComponent::g_Name = "BrickRenderComponent";
...
void BaseRenderComponent::VPostInit(void)
{
    ...
    shared_ptr<EvtData_New_Render_Component> pEvent(AC_NEW EvtData_New_Render_Component(m_pOwner->GetId(), pSceneNode));
...
{
    shared_ptr<EvtData_Modified_Render_Component> pEvent(AC_NEW EvtData_Modified_Render_Component(m_pOwner->GetId()));
    IEventManager::Get()->VTriggerEvent(pEvent);
}
...
TiXmlElement* BaseRenderComponent::VGenerateXml(void)
{
    ...
    // color
    TiXmlElement* pColor = AC_NEW TiXmlElement("Color");
...
{
    AC_ERROR("MeshRenderComponent::VGenerateSubclassXml() not implemented");
}

//---------------------------------------------------------------------------------------------------------------------
// BrickRenderComponent
//---------------------------------------------------------------------------------------------------------------------
BrickRenderComponent::BrickRenderComponent(void)
{
    m_width = 1.0f; 
	m_length = 2.0f, 
	m_height = 1.0f;
...
bool BrickRenderComponent::VDelegateInit(TiXmlElement* pData)
{
	TiXmlElement* pSize = pData->FirstChildElement("Dimensions");
    if (pSize)
	{
		double w = 1.0;
		double l = 1.0;
...
shared_ptr<SceneNode> BrickRenderComponent::VCreateSceneNode(void)
{
	...
	//if (AlphaApp::GetRendererImpl()==GameCodeApp::Renderer_D3D9)
	//{
	...
	//	shared_ptr<SceneNode> sphere(AC_NEW D3DShaderMeshNode9(m_pOwner->GetId(), wbrcp, pSphereMesh, "Effects\\GameCode4.fx", RenderPass_Actor, &pTransformComponent->GetTransform()));
	...
	//{
		shared_ptr<SceneNode> brick(AC_NEW BrickNodeInstance(m_pOwner->GetId(), wbrcp, &pTransformComponent->GetTransform()));
		return brick;
	...
	//}
	return shared_ptr<SceneNode>(NULL); // removeonce i make the sphere mesh
}

void BrickRenderComponent::VCreateInheritedXmlElements(TiXmlElement* pBaseElement)
{
	TiXmlElement* pSize = AC_NEW TiXmlElement("Size");
    pSize->SetAttribute("l", ToStr(m_width).c_str());
    pSize->SetAttribute("w", ToStr(m_length).c_str());
    pSize->SetAttribute("b", ToStr(m_height).c_str());
    pBaseElement->LinkEndChild(pSize);

	TiXmlElement* pTextureNode = AC_NEW TiXmlElement("Texture");
    TiXmlText* pTextureText = AC_NEW TiXmlText(m_textureResource.c_str());
...
shared_ptr<SceneNode> SphereRenderComponent::VCreateSceneNode(void)
{
	...
	//{
		...
		//shared_ptr<SceneNode> sphere(AC_NEW D3DShaderMeshNode11(m_pOwner->GetId(), wbrcp, "art\\sphere.sdkmesh", RenderPass_Actor, &pTransformComponent->GetTransform()));
		//return sphere;
	...
	//	return shared_ptr<SceneNode>(NULL);
	//}
	return shared_ptr<SceneNode>(); // removeonce i make the sphere mesh
}

void SphereRenderComponent::VCreateInheritedXmlElements(TiXmlElement* pBaseElement)
{
    TiXmlElement* pMesh = AC_NEW TiXmlElement("Sphere");
...
shared_ptr<SceneNode> TeapotRenderComponent::VCreateSceneNode(void)
{
    ...
    if (pTransformComponent)
    {
		...
		//switch (AlhpaApp::GetRendererImpl())
		//{
		//	case AlhpaApp::Renderer_D3D9: 
		//		return shared_ptr<SceneNode>(AC_NEW D3DTeapotMeshNode9(m_pOwner->GetId(), weakThis, "Effects\\GameCode4.fx", RenderPass_Actor, &pTransformComponent->GetTransform()));

		//	case AlhpaApp::Renderer_D3D11: 
		//	{
				Mat4x4 rot90;
				rot90.BuildRotationY(-GCC_PI/2.0f);
				shared_ptr<SceneNode> parent(AC_NEW SceneNode(m_pOwner->GetId(), weakThis, RenderPass_Actor, &pTransformComponent->GetTransform()));
				//shared_ptr<SceneNode> teapot(AC_NEW D3DTeapotMeshNode11(INVALID_ACTOR_ID, weakThis, RenderPass_Actor, &rot90)); // un comment when i have made the Teapot mesh
...
shared_ptr<SceneNode> GridRenderComponent::VCreateSceneNode(void)
{
    ...
    if (pTransformComponent)
    {
		...
		//switch (AlphaApp::GetRendererImpl())
		//{
		//	case AlphaApp::Renderer_D3D9: 
		//		return shared_ptr<SceneNode>(AC_NEW D3DGrid9(m_pOwner->GetId(), weakThis, &(pTransformComponent->GetTransform())));  

		//	case AlphaApp::Renderer_D3D11: 
				return shared_ptr<SceneNode>(AC_NEW D3DGrid11(m_pOwner->GetId(), weakThis, &(pTransformComponent->GetTransform())));  
								 
		//	default:
		//		AC_ERROR("Unknown Renderer Implementation in GridRenderComponent");
		//}
    }
...
void GridRenderComponent::VCreateInheritedXmlElements(TiXmlElement *pBaseElement)
{
    ...
    TiXmlElement* pDivisionNode = AC_NEW TiXmlElement("Division");
    TiXmlText* pDivisionText = AC_NEW TiXmlText(ToStr(m_squares).c_str());
...
shared_ptr<SceneNode> LightRenderComponent::VCreateSceneNode(void)
{
    ...
    if (pTransformComponent)
    {
		...
		//{
		//	case AlphaApp::Renderer_D3D9: 
		//		return shared_ptr<SceneNode>(AC_NEW D3DLightNode9(m_pOwner->GetId(), weakThis, m_Props, &(pTransformComponent->GetTransform())));  

		//	case AlphaApp::Renderer_D3D11: 
				return shared_ptr<SceneNode>(AC_NEW D3DLightNode11(m_pOwner->GetId(), weakThis, m_Props, &(pTransformComponent->GetTransform())));  
								 
		//	default:
		//		AC_ASSERT(0 && "Unknown Renderer Implementation in GridRenderComponent");
		//}
	}
    return shared_ptr<SceneNode>();
}
...
{
    TiXmlElement* pSceneNode = AC_NEW TiXmlElement("Light");

    // attenuation
    TiXmlElement* pAttenuation = AC_NEW TiXmlElement("Attenuation");
    ...
    // shape
    TiXmlElement* pShape = AC_NEW TiXmlElement("Shape");
...
shared_ptr<SceneNode> SkyRenderComponent::VCreateSceneNode(void)
{
	...
	//if (AlphaApp::GetRendererImpl()==GameCodeApp::Renderer_D3D9)
	//{
	//	sky = shared_ptr<SkyNode>(AC_NEW D3DSkyNode9(m_textureResource.c_str() ));
	//}
	//else if (AlphaApp::GetRendererImpl()==GameCodeApp::Renderer_D3D11)
	//{
		sky = shared_ptr<SkyNode>(AC_NEW D3DSkyNode11(m_textureResource.c_str() ));
	//}
	//else
	//{
	//	AC_ERROR("Unknown Renderer Implementation in VLoadGameDelegate");
	//}
	return sky;
}
