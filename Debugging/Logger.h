...
// display flags
const unsigned char LOGFLAG_WRITE_TO_LOG_FILE =         1 << 1;
...
#define AC_FATAL(str) \
	do \
	{ \
		static Logger::ErrorMessenger* pErrorMessenger = AC_NEW Logger::ErrorMessenger; \
		std::string s((str)); \
...
#define AC_ERROR(str) do { (void)sizeof(str); } while(0) 
#define AC_WARNING(str) do { (void)sizeof(str); } while(0) 
#define AC_INFO(str) do { (void)sizeof(str); } while(0) 
#define AC_LOG(tag, str) do { (void)sizeof(tag); (void)sizeof(str); } while(0) 
#define AC_ASSERT(expr) do { (void)sizeof(expr); } while(0) 
