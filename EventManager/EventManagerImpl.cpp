...
{
    AC_LOG("Events", "Attempting to add delegate function for event type: " + ToStr(type, 16));
    ...
    for (auto it = eventListenerList.begin(); it != eventListenerList.end(); ++it)
    {
        if (eventDelegate == (*it))
        {
            AC_WARNING("Attempting to double-register a delegate");
            return false;
        }
    }

    eventListenerList.push_back(eventDelegate);
    AC_LOG("Events", "Successfully added delegate for event type: " + ToStr(type, 16));
...
{
    AC_LOG("Events", "Attempting to remove delegate function from event type: " + ToStr(type, 16));
        bool success = false;
    ...
    if (findIt != m_eventListeners.end())
    {
        ...
        for (auto it = listeners.begin(); it != listeners.end(); ++it)
        {
            ...
            {
                listeners.erase(it);
                AC_LOG("Events", "Successfully removed delegate function from event type: " + ToStr(type, 16));
                success = true;
...
{
    AC_LOG("Events", "Attempting to trigger event " + std::string(pEvent->GetName()));
    ...
    if(findIt != m_eventListeners.end())
    {
        ...
        for(EventListenerList::const_iterator it = eventListenerList.begin(); it != eventListenerList.end(); ++it)
        {
			...
			AC_LOG("Events", "Sending Event " + std::string(pEvent->GetName()) + " to delegate.");
...
{
    AC_ASSERT(m_activeQueue >= 0);
    AC_ASSERT(m_activeQueue < EVENTMANAGER_NUM_QUEUES);

    // make sure the event is valid
    if(!pEvent)
    {
        AC_ERROR("Invalid event in VQueueEvent()");
        return false;
    }

    AC_LOG("Events", "Attempting to queue event: " + std::string(pEvent->GetName()));

    auto findIt = m_eventListeners.find(pEvent->VGetEventType());
    if(findIt != m_eventListeners.end())
    {
        m_queues[m_activeQueue].push_back(pEvent);
        AC_LOG("Events", "Successfully queued event: " + std::string(pEvent->GetName()));
        return true;
    }
    else
    {
        AC_LOG("Events", "Skipping event since there are no delegates registered to receive it: " + std::string(pEvent->GetName()));
        return false;
    }
}
...
bool EventManager::VUpdate(unsigned long maxMillis)
{
	...
	while (m_realtimeEventQueue.try_pop(pRealtimeEvent))
	{
			...
			if (maxMillis != IEventManager::kINFINITE)
			{
					if (currMs >= maxMs)
					{
							AC_ERROR("A realtime process is spamming the event manager!");
					}
			}
	}
	...
	AC_LOG("EventLoop", "Processing Event Queue " + ToStr(queueToProcess) + "; " + ToStr((unsigned long)m_queues[queueToProcess].size()) + " events to process");
	...
	while (!m_queues[queueToProcess].empty())
	{
		...
		AC_LOG("EventLoop", "\t\tProcessing Event " + std::string(pEvent->GetName()));
		...
		if (findIt != m_eventListeners.end())
		{
			...
			AC_LOG("EventLoop", "\t\tFound " + ToStr((unsigned long)eventListeners.size()) + " delegates");

			// call each listener
			for (auto it = eventListeners.begin(); it != eventListeners.end(); ++it)
			{
				EventListenerDelegate listener = (*it);
				AC_LOG("EventLoop", "\t\tSending event " + std::string(pEvent->GetName()) + " to delegate");
				listener(pEvent);
			}
		}
		...
		{
			AC_LOG("EventLoop", "Aborting event processing; time ran out");
			break;
		}
	}
