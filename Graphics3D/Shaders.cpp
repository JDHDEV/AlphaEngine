...
//
// Alpha_Hlsl_VertexShader::Alpha_Hlsl_VertexShader                     - Chapter 15, page 508
//
Alpha_Hlsl_VertexShader::Alpha_Hlsl_VertexShader()
{
...
//
// Alpha_Hlsl_VertexShader::SetupRender                                     - Chapter 15, page 513
//
HRESULT Alpha_Hlsl_VertexShader::SetupRender(Scene *pScene, SceneNode *pNode)
{
	HRESULT hr = S_OK;
...
//
// Alpha_Hlsl_PixelShader::Alpha_Hlsl_PixelShader       - Chapter 15, page 516
//
Alpha_Hlsl_PixelShader::Alpha_Hlsl_PixelShader()
{
    m_pPixelShader = NULL;
...
//
// Alpha_Hlsl_PixelShader::OnRestore                                - Chapter 15, page 516
//
HRESULT Alpha_Hlsl_PixelShader::OnRestore(Scene *pScene)
{
    HRESULT hr = S_OK;
    ...
    std::string hlslFileName = "Effects\\Alpha_PS.hlsl";
    Resource resource(hlslFileName.c_str());
    ...
    if(FAILED(d3dRenderer11->CompileShader(pResourceHandle->Buffer(), pResourceHandle->Size(), hlslFileName.c_str(), "Alpha_PSMain", "ps_4_0_level_9_1", &pPixelShaderBuffer)))
    {
...
//
// Alpha_Hlsl_PixelShader::SetupRender                              - Chapter 15, page 518
//
HRESULT Alpha_Hlsl_PixelShader::SetupRender(Scene *pScene, SceneNode *pNode)
{
    HRESULT hr = S_OK;
	...
	g_pDX11W->GetD3D11DeviceContext()->PSSetConstantBuffers( 0, 1, &m_pcbPSMaterial );
...
//
// Alpha_Hlsl_PixelShader::SetTexture                               - Chapter 15, page 519
//
HRESULT Alpha_Hlsl_PixelShader::SetTexture(const std::string& textureName)
{
...
HRESULT Alpha_Hlsl_PixelShader::SetTexture(ID3D11ShaderResourceView* const *pDiffuseRV, ID3D11SamplerState * const *ppSamplers)
{
...
HRESULT LineDraw_Hlsl_Shader::SetDiffuse(const std::string& textureName, const Color &color)
{
	...
	// ----- Diffuse Color ------
	V(g_pDX11W->GetD3D11DeviceContext()->Map(m_pcbDiffuseColor, 0, D3D11_MAP_WRITE_DISCARD, 0, &MappedResource));
	Vec4 * pDiffuseColor = (Vec4*)MappedResource.pData;
	*pDiffuseColor = Vec4(color.r, color.g, color.b, color.a);
	g_pDX11W->GetD3D11DeviceContext()->Unmap(m_pcbDiffuseColor, 0);
...
//
// Alpha_Hlsl_LineShader::Alpha_Hlsl_LineShader
//
Alpha_Hlsl_LineShader::Alpha_Hlsl_LineShader()
{
    m_pVertexLayout11 = NULL;
...
//
// Alpha_Hlsl_LineShader::~Alpha_Hlsl_LineShader            - Chapter 15, page 508
//
Alpha_Hlsl_LineShader::~Alpha_Hlsl_LineShader()
{
    SAFE_RELEASE(m_pVertexLayout11);
...
//
// Alpha_Hlsl_LineShader::SetupRender                                     - Chapter 15, page 513
//
HRESULT Alpha_Hlsl_LineShader::SetupRender(Scene *pScene)
{
	HRESULT hr = S_OK;
    ...
    V(g_pDX11W->GetD3D11DeviceContext()->Map(m_pcbPSMaterial, 0, D3D11_MAP_WRITE_DISCARD, 0, &MappedResource2));
    ConstantBuffer_Material* pPSMaterial2 = (ConstantBuffer_Material*)MappedResource2.pData;

    pPSMaterial2->m_vDiffuseObjectColor = Vec4(color.r, color.g, color.b, color.a);

    if (m_textureResource.length() > 0)
    {
        pPSMaterial2->m_bHasTexture = true;
    }
    else
    {
        pPSMaterial2->m_bHasTexture = false;
    }
...
HRESULT Alpha_Hlsl_LineShader::SetTexture(ID3D11ShaderResourceView* const *pDiffuseRV, ID3D11SamplerState * const *ppSamplers)
{
    g_pDX11W->GetD3D11DeviceContext()->PSSetShaderResources(0, 1, pDiffuseRV);
    g_pDX11W->GetD3D11DeviceContext()->PSSetSamplers(0, 1, ppSamplers);
    return S_OK;
