//========================================================================
// LuaStateManager.cpp : Implements the LuaStateManager class.
//
...
bool LuaStateManager::Create(void)
{
    if (s_pSingleton)
    {
        AC_ERROR("Overwriting LuaStateManager singleton");
        SAFE_DELETE(s_pSingleton);
    }

    s_pSingleton = AC_NEW LuaStateManager;
    if (s_pSingleton)
...
LuaPlus::LuaObject LuaStateManager::CreatePath(const char* pathString, bool toIgnoreLastElement)
{
    ...
    for (auto it = splitPath.begin(); it != splitPath.end(); ++it)
    {
        ...
        if (context.IsNil())
        {
            AC_ERROR("Something broke in CreatePath(); bailing out (element == " + (*it) + ")");
        ...
        if (!curr.IsTable())
        {
            ...
            if (!curr.IsNil())
            {
                AC_WARNING("Overwriting element '" + element + "' in table");
...
void LuaStateManager::ConvertTableToVec3(const LuaPlus::LuaObject& luaTable, Vec3& outVec3) const
{
    ...
    if (temp.IsNumber())
        ...
        AC_ERROR("luaTable.x is not a number");
    ...
    if (temp.IsNumber())
        ...
        AC_ERROR("luaTable.y is not a number");
    ...
    if (temp.IsNumber())
        ...
        AC_ERROR("luaTable.z is not a number");
}