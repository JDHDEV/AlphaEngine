...
void ScriptEventListenerMgr::DestroyListener(ScriptEventListener* pListener)
{
	...
	{
		AC_ERROR("Couldn't find script listener in set; this will probably cause a memory leak");
	}
}
...
{
    AC_ASSERT(m_scriptCallbackFunction.IsFunction());  // this should never happen since it's validated before even creating this object
...
{
	AC_ASSERT(s_pScriptEventListenerMgr == NULL);
	s_pScriptEventListenerMgr = AC_NEW ScriptEventListenerMgr;
...
{
	AC_ASSERT(s_pScriptEventListenerMgr);

	if (callbackFunction.IsFunction())
	{
		// create the C++ listener proxy and set it to listen for the event
		ScriptEventListener* pListener = AC_NEW ScriptEventListener(eventType, callbackFunction);
	...
	AC_ERROR("Attempting to register script event listener with invalid callback function");
	return 0;
}
...
void InternalScriptExports::AttachScriptProcess(LuaPlus::LuaObject scriptProcess)
{
	...
	{
		AC_ERROR("Couldn't find __object in script process");
	}
}
...
int InternalScriptExports::CreateActor(const char* actorArchetype, LuaPlus::LuaObject luaPosition, LuaPlus::LuaObject luaYawPitchRoll)
{
    ...
    {
	    AC_ERROR("Invalid object passed to CreateActor(); type = " + std::string(luaPosition.TypeName()));
		return INVALID_ACTOR_ID;
	}

    if (!luaYawPitchRoll.IsTable())
    {
	    AC_ERROR("Invalid object passed to CreateActor(); type = " + std::string(luaYawPitchRoll.TypeName()));
		return INVALID_ACTOR_ID;
	}
...
float InternalScriptExports::GetYRotationFromVector(LuaPlus::LuaObject vec3)
{
    ...
    AC_ERROR("Invalid object passed to GetYRotationFromVector(); type = " + std::string(vec3.TypeName()));
    return 0;
}
...
void InternalScriptExports::LuaLog(LuaPlus::LuaObject text)
{
    ...
    {
        AC_LOG("Lua", text.ToString());
    }
    else
    {
        AC_LOG("Lua", "<" + std::string(text.TypeName()) + ">");
    }
}
...
//		return;
//    }
//    AC_ERROR("Invalid object passed to ApplyForce(); type = " + std::string(normalDirLua.TypeName()));
//}
//
...
//		return;
//    }
//    AC_ERROR("Invalid object passed to ApplyTorque(); type = " + std::string(axisLua.TypeName()));
//}
