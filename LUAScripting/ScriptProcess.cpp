...
bool ScriptProcess::BuildCppDataFromScript(LuaPlus::LuaObject scriptClass, LuaPlus::LuaObject constructionData)
{
	if (scriptClass.IsTable())
	{
		...
		{
			AC_ERROR("No OnUpdate() found in script process; type == " + std::string(temp.TypeName()));
	...
	{
	    AC_ERROR("scriptClass is not a table in ScriptProcess::BuildCppDataFromScript()");
	    return false;
	}
...
void ScriptProcess::ScriptAttachChild(LuaPlus::LuaObject child)
{
    if (child.IsTable())
    {
        ...
        {
            AC_ERROR("Attempting to attach child to ScriptProcess with no valid __object");
        }
    }
    else
    {
        AC_ERROR("Invalid object type passed into ScriptProcess::ScriptAttachChild(); type = " + std::string(child.TypeName()));
    }
}
...
LuaPlus::LuaObject ScriptProcess::CreateFromScript(LuaPlus::LuaObject self, LuaPlus::LuaObject constructionData, LuaPlus::LuaObject originalSubClass)
{
	...
	AC_LOG("Script", std::string("Creating instance of ") + SCRIPT_PROCESS_NAME);
	ScriptProcess* pObj = AC_NEW ScriptProcess;
