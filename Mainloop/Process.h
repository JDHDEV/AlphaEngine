...
{
    AC_ASSERT(m_state == RUNNING || m_state == PAUSED);
    m_state = SUCCEEDED;
}
...
inline void Process::Pause(void)
{
	if (m_state == RUNNING)
		...
		AC_WARNING("Attempting to pause a process that isn't running");
}
...
inline void Process::UnPause(void)
{
    if (m_state == PAUSED)
        ...
        AC_WARNING("Attempting to unpause a process that isn't paused");
}
