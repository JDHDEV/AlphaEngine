...
//
/////////////////////////////////////////////////////////////////////////////
class BulletPhysics : public IGamePhysics, AC_noncopyable
{
...
bool BulletPhysics::VInitialize()
{
	...
	m_collisionConfiguration = AC_NEW btDefaultCollisionConfiguration();

	// this manages how Bullet detects precise collisions between pairs of objects
	m_dispatcher = AC_NEW btCollisionDispatcher( m_collisionConfiguration);
	...
	m_broadphase = AC_NEW btDbvtBroadphase();
		...
		if (pGameActor && actorMotionState)
		{
            ...
            if (pTransformComponent)
            {
			    if (pTransformComponent->GetTransform() != actorMotionState->m_worldToPositionTransform)
                {
                    ...
                    shared_ptr<EvtData_Move_Actor> pEvent(AC_NEW EvtData_Move_Actor(id, actorMotionState->m_worldToPositionTransform));
...
{
    AC_ASSERT(pGameActor);

    ActorId actorID = pGameActor->GetId();
	AC_ASSERT(m_actorIdToRigidBody.find( actorID ) == m_actorIdToRigidBody.end() && "Actor with more than one physics body?");
	...
	ActorMotionState * const myMotionState = AC_NEW ActorMotionState(transform);
...
void BulletPhysics::VAddBox(const Vec3& dimensions, WeakActorPtr pGameActor, const std::string& densityStr, const std::string& physicsMaterial)
{
	...
	// create the collision body, which specifies the shape of the object
	btVector3 btv = Vec3_to_btVector3(dimensions);
	btBoxShape * const boxShape = new btBoxShape(btv);
...
void BulletPhysics::VCreateTrigger(WeakActorPtr pGameActor, const Vec3 &pos, const float dim)
{
	...
	ActorMotionState * const myMotionState = AC_NEW ActorMotionState( triggerTrans );
...
void BulletPhysics::BulletInternalTickCallback( btDynamicsWorld * const world, btScalar const timeStep )
{
	...
	AC_ASSERT( world->getWorldUserInfo() );
...
void BulletPhysics::SendCollisionPairAddEvent( btPersistentManifold const * manifold, btRigidBody const * const body0, btRigidBody const * const body1 )
{
	if ( body0->getUserPointer() || body1->getUserPointer() )
	{
        ...
        shared_ptr<EvtData_PhysTrigger_Enter> pEvent(AC_NEW EvtData_PhysTrigger_Enter(triggerId, FindActorID(otherBody)));
        IEventManager::Get()->VQueueEvent(pEvent);
	}
	else
	{
        ...
        shared_ptr<EvtData_PhysCollision> pEvent(AC_NEW EvtData_PhysCollision(id0, id1, sumNormalForce, sumFrictionForce, collisionPoints));
        IEventManager::Get()->VQueueEvent(pEvent);
	}
}
...
void BulletPhysics::SendCollisionPairRemoveEvent( btRigidBody const * const body0, btRigidBody const * const body1 )
{
	if ( body0->getUserPointer() || body1->getUserPointer() )
	{
		...
		// send the trigger event.
		int const triggerId = *static_cast<int*>(triggerBody->getUserPointer());
        shared_ptr<EvtData_PhysTrigger_Leave> pEvent(AC_NEW EvtData_PhysTrigger_Leave(triggerId, FindActorID( otherBody)));
        IEventManager::Get()->VQueueEvent(pEvent);
	}
	else
	{
        ...
        shared_ptr<EvtData_PhysSeparation> pEvent(AC_NEW EvtData_PhysSeparation(id0, id1));
        IEventManager::Get()->VQueueEvent(pEvent);
	}
}
...
IGamePhysics *CreateGamePhysics()
{
	...
	gamePhysics.reset( AC_NEW BulletPhysics );
...
{
	std::auto_ptr<IGamePhysics> gamePhysics;
	gamePhysics.reset( AC_NEW NullPhysics );
	if (gamePhysics.get() && !gamePhysics->VInitialize())
	{
		// physics failed to initialize.  delete it.
		gamePhysics.reset();
