...
bool DevelopmentResourceZipFile::VOpen()
{
	...
	else
	{
		...
		AC_ASSERT(0 && "Not implemented yet");
	}
...
{ 
	...
	if ( m_file->VOpen() )
	{
		RegisterLoader(shared_ptr<IResourceLoader>(AC_NEW DefaultResourceLoader()));
		retValue = true;
	}
	return retValue;
}
...
shared_ptr<ResHandle> ResCache::Load(Resource *r)
{
	...
	if (!loader)
	{
		AC_ASSERT(loader && _T("Default resource loader not found!"));
	...
	if (rawSize < 0)
	{
		AC_ASSERT(rawSize > 0 && "Resource size returned -1 - Resource not found");
		return shared_ptr<ResHandle>();
	}

    int allocSize = rawSize + ((loader->VAddNullZero()) ? (1) : (0));
	char *rawBuffer = loader->VUseRawFile() ? Allocate(allocSize) : AC_NEW char[allocSize];
	...
	{
		buffer = rawBuffer;
		handle = shared_ptr<ResHandle>(AC_NEW ResHandle(*r, buffer, rawSize, this));
	}
	else
	{
		...
		handle = shared_ptr<ResHandle>(AC_NEW ResHandle(*r, buffer, size, this));
