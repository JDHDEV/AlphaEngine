...
bool ZipFile::Init(const std::wstring &resFileName)
{
  ...
  m_pDirData = AC_NEW char[dh.dirSize + dh.nDirEntries*sizeof(*m_papDir)];
  if (!m_pDirData)
    return false;
...
bool ZipFile::ReadFile(int i, void *pBuf)
{
  ...
  char *pcData = AC_NEW char[h.cSize];
  if (!pcData)
    return false;
  ...
  if (err == Z_OK)
  {
	  ...
	  bool cancel = false;
		while (stream.total_in < (uInt)h.cSize && !cancel)
		{
			...
			{
				AC_ASSERT(0 && "Something happened.");
				break;
			}
