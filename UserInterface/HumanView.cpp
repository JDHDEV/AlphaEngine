...
{
 //	m_currTick = timeGetTime(); this dosent wokr maybe cus WIN32_LEAN_AND_MEAN but GC4 made it work somehow TODO
	//if (m_currTick == m_lastDraw)
	//	return;

	m_currTick = g_pDX11W->GetTime();
	double framerate = 1.0/120.0;
	// It is time to draw ?
	//if( m_runFullSpeed || ( (m_currTick - m_lastDraw) > 1/120))//SCREEN_REFRESH_RATE) )
	//if((m_currTick - m_lastDraw) > framerate)
	//{
		g_pDX11W->DX11WUpdateFrameStats();
...
bool HumanView::InitAudio()
{
        if (!g_pAudio)
        {
                g_pAudio = AC_NEW DirectSoundAudio();          // use this line for DirectSound
        }
...
void HumanView::PlaySoundDelegate(IEventDataPtr pEventData)
{
    ...
    shared_ptr<SoundProcess> sfx(AC_NEW SoundProcess(srh, 100, false));
