#pragma once
//========================================================================
// DirectX11Timer.cpp : Shamelessly stolen from DXUT i hope they forgive me
...
//========================================================================
#include "AlphaStd.h"
#include "AlphaTimer.h"

AlphaTimer* WINAPI GetGlobalAlphaTimer()
{
    // Using an accessor function gives control of the construction order
    static AlphaTimer timer;
    return &timer;
}

AlphaTimer::AlphaTimer()
{
...
void AlphaTimer::Advance()
{
    m_llStopTime += m_llQPFTicksPerSec / 10;
}

double AlphaTimer::GetAbsoluteTime() const
{
    LARGE_INTEGER qwTime = { 0 };
    QueryPerformanceCounter( &qwTime );
...
double AlphaTimer::GetTime() const
{
    LARGE_INTEGER qwTime = GetAdjustedCurrentTime();
...
void AlphaTimer::GetTimeValues( double* pfTime, double* pfAbsoluteTime, float* pfElapsedTime )
{
...
float AlphaTimer::GetElapsedTime()
{
    LARGE_INTEGER qwTime = GetAdjustedCurrentTime();
    ...
    // See the explanation about clamping in AlphaTimer::GetTimeValues()
    if( fElapsedTime < 0.0f )
        fElapsedTime = 0.0f;
...
//--------------------------------------------------------------------------------------
LARGE_INTEGER AlphaTimer::GetAdjustedCurrentTime() const
{
...
//--------------------------------------------------------------------------------------
void AlphaTimer::LimitThreadAffinityToCurrentProc()
{
